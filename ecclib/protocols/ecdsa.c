                ...
                const eccp_parameters_t* param)
{
  ...
  do
  {
    do
    {
      ...
      eccp_jacobian_point_multiply_L2R_DA(&ephemeral_point, &param->base_point, ephemeral_key,
                                          param);
    ...
    gfp_normal_to_montgomery(ephemeral_point.y, private_key,
                             &param->order_n_data); // d * R^2 * R^-1
    gfp_mont_multiply(ephemeral_point.x, ephemeral_point.y, ephemeral_key,
                      &param->order_n_data); // d R * k^-1 R * R^-1
    gfp_mont_multiply(ephemeral_point.x, ephemeral_point.x, signature->r,
                      &param->order_n_data); // d k^-1 R * r * R^-1
    gfp_mont_multiply(ephemeral_point.y, ephemeral_key, hash_of_message,
                      &param->order_n_data); // k^-1 R * e * R^-1
    gfp_gen_add(signature->s, ephemeral_point.x, ephemeral_point.y,
                &param->order_n_data); // k^-1 (e+d*r)
 ...
 * @param hash_of_message the hash of the message (smaller than param->order_n_data.prime)
 * @param public_key the public key used for the verification (assumed to be not in montgomery
 * domain)
 * @param param elliptic curve parameters
                   ...
                   const eccp_point_affine_t* public_key, const eccp_parameters_t* param)
{
  ...
  /* Verify that r and s are integers in the interval [1,n-1]. If any verification fails then reject
   * the signature */
 ...
 */
void ecdsa_hash_to_gfp(gfp_t element, const uint8_t* hash, const size_t hash_bits,
                       const gfp_prime_data_t* prime)
{
  size_t hash_bytes = BYTES_PER_BITS(hash_bits);
  size_t i;
  ...
  else
  {
    ...
    size_t prime_bytes = BYTES_PER_BITS(prime->bits);
