 ...
 * @param param elliptic curve parameters
    ...
    Source: 2007 Bernstein-Lange; note that the improvement from 12M+4S to 11M+5S was already
 mentioned in 2001 Bernstein
                                          ...
                                          const gfp_t scalar, const eccp_parameters_t* param)
{
  ...
  scalar_helper[scalar_helper_size - 1] +=
      bigint_add_var(scalar_helper, scalar_helper, scalar, param->order_n_data.words);
                                       ...
                                       const gfp_t scalar, const eccp_parameters_t* param)
{
  ...
  int digit;
  size_t j, j_cnt;
  size_t comb_param_d = (param->order_n_data.bits - 1) / width + 1; // same as ceil (bits / width)
  result_projective.identity = 1;
  ...
  while (digit >= 0)
  {
    ...
    {
      j |= bigint_test_bit_var(scalar, comb_param_d * j_cnt + digit, param->order_n_data.words)
           << j_cnt;
    }

    if (j > 0)
    {
      eccp_jacobian_point_add_affine(&result_projective, &result_projective, &P_table[j - 1],
                                     param);
    }
    digit--;
  }
